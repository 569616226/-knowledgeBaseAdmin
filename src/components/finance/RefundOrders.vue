<template>
	<div>
		...
		<div class="know-container">
			<data-tables v-loading="loading" :data="RefundOrdersData" :table-props="tableProps" align="left" :show-action-bar="false" :custom-filters="customFilters" :pagination-def="paginationDef">
				<el-row slot="custom-tool-bar" style="margin-bottom: 10px" class="class_el_row">
					<el-col :span="24" class="text-align-left">
						...
						<el-select v-model="customFilters[1].vals" placeholder="退款类型" class="class_select_width">
							<el-option label="退款类型" value=""></el-option>
							<div v-for="status in Typestatus">
								<el-option :label="status" :value="status"></el-option>
							</div>
						</el-select>
						<el-select v-model="customFilters[2].vals" placeholder="审核状态" class="class_select_width">
				...
				<el-table-column prop="refund_type" label="退款类型" key="refund_type" sortable="custom">
				</el-table-column>
				<el-table-column prop="guest_name" label="退款人" key="guest_name" sortable="custom">
					<template slot-scope="scope">
						<router-link :to="{name: 'RefundDetail',params: {id: scope.row.id}}">
				...
				<el-table-column prop="status" label="转账状态" key="status" sortable="custom">
					<template slot-scope="scope">
						<div v-if="scope.row.status == '退款中'" class="gray">{{scope.row.status}}</div>
						<div v-if="scope.row.status == '已退款'" class="yellow">{{scope.row.status}}</div>
					</template>
				</el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getData () {
				if(this.RefundOrdersData.length == 0){
					this.loading = true
				}
				await this.getRefundOrders()
				this.loading = false
			}
		},
		mounted() {
