<template>
	<div>
		...
		<div class="know-container">
			<data-tables v-loading="loading" :data="GuestsData" :table-props="tableProps" align="left" :show-action-bar="false" :custom-filters="customFilters" :pagination-def="paginationDef">
				<el-row slot="custom-tool-bar" style="margin-bottom: 10px" class="class_el_row">
					<el-col :span="24" class="text-align-left">
						...
						<el-input v-model="customFilters[0].vals" prefix-icon="el-icon-search" placeholder="搜索用户Id/手机号码" class="class_input_width">
						</el-input>
					</el-col>
				</el-row>
				<el-table-column prop="name" label="用户id" key="name" sortable="custom">
					<template slot-scope="scope">
						<router-link :to="{name: 'GuestsDetail',params: {id: scope.row.id}}">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getData() {
				if(this.GroupsData.length == 0 || this.GuestsData.length == 0) {
					this.loading = true
				}
				await this.getGroups()
